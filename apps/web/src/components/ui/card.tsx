...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-4 rounded-2xl border border-border/60 bg-card/80 p-4 text-card-foreground shadow-sm",
        className
      )}
      {...props}
    />
...
function CardInset({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("rounded-xl bg-muted/50 p-3", className)}
      {...props}
    />
  );
}
